...
Inc_Done:
	...
	mov a, Count1ms+0
	cjne a, #low(1000), Timer2_ISR_done_5 ; Warning: this instruction changes the carry flag!
	mov a, Count1ms+1
	cjne a, #high(1000), Timer2_ISR_done_5
	
	clr a 
	jb BOOT_BUTTON, NextLinePleaseZero
	Wait_Milli_Seconds(#50)


	;generates an 8 bit pseudo-random number which is returned in Acc.
	;one byte of memory must be available for rand8reg
	;CODE IS BUILT UPON: http://www.8051projects.info/threads/random-a-number-in-assembly-language.625/ 
	rand8: mov a, Clock_CounterSeconds
	jnz rand8b
	cpl a
	...
	rand9: mov a, Clock_CounterMinutes
	jnz rand9b
	cpl a
	mov Clock_CounterMinutes, a
	rand9b: anl a, #10111000b
	mov c, p
	...
	rand7: mov a, Clock_CounterHours
	jnz rand7b
	cpl a
	mov Clock_CounterHours, a
	rand7b: anl a, #10111000b
	mov c, p
...
NextLinePleaseZero:
	...
	clr a
	mov a, SimpleCounter
	cjne a, #0x01, NextLinePlease11
	mov a, #0x00
	add a, BCD_CounterHours
	subb a, Clock_CounterHours
	cjne a, #0x00, NextLinePlease11
	mov a, #0x00
...
NextLinePlease11:
	clr TR0
	cjne a, #0x99, NextLinePlease24
Timer2_ISR_done_5:
	ljmp Timer2_ISR_done_4
NextLinePlease24:
Bob:
